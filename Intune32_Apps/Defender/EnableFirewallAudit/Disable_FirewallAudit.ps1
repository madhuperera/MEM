...
(
    ...
    [String] $S_ScriptName = "FirewallAuditDisable"
)
...
try
{
    ...
    if ($current."Inclusion Setting" -eq "failure")
    {
        Write-Host "Disabling. $($current | ForEach-Object {$_.Subcategory + ":" + $_.'Inclusion Setting' + ";"})"
        $output = auditpol /set /subcategory:"$($categories)" /failure:disable
