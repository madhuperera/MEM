**Warning: This Help file was generated with AI.**
    ...
    else
    {
        try
        {
            ...
            if ($WingetPInstallerPath)
            {
                ...
                $ArgsList = "install" + ' ' + $S_WingetAppID + ' ' + '--silent' + ' ' + '--accept-source-agreements' + ' ' + '--accept-package-agreements'
                Start-Process -FilePath ".\winget.exe" -ArgumentList $ArgsList -WindowStyle Hidden -Wait -ErrorAction Stop
...
else
{
    if (Test-WingetAppInstalled -F_WingetAppId $S_WingetAppID)
    {
        try
        {
            ...
            {
                Set-Location -Path $WingetPInstallerPath -ErrorAction Stop
                $ArgsList = "uninstall" + ' ' + $S_WingetAppID + ' ' + '--silent'
                Start-Process -FilePath ".\winget.exe" -ArgumentList $ArgsList -WindowStyle Hidden -Wait -ErrorAction Stop
