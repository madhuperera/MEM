<#
.SYNOPSIS
    Detects if any L2TP VPN connections exist in the current user's phonebook.

.DESCRIPTION
    This detection script checks for the presence of any VPN connections using the L2TP tunnel type
    in the current user's VPN phonebook. If any L2TP connections are found, the script returns
...
try
{
    ...
    if ($L2tpConnections)
    {
        ...
        Write-Output "Non-Compliant: Found L2TP VPN connection(s) in user phonebook: $Names"
        exit 1
    }
    else
    {
        Write-Output "Compliant: No L2TP VPN connections found in user phonebook."
        exit 0
    }
}
catch
