...
.DESCRIPTION
    ...
    For every file and folder under each known folder it checks:
      ...
      * Reparse points - junctions and symbolic links, which Known Folder Move refuses to
        move. OneDrive Files On-Demand placeholders are also reparse points but carry a
        different reparse tag; the scan reads the tag (PowerShell's LinkType) so
        placeholders - hydrated or online-only - are scanned as ordinary content.
    ...
    contains one. Office owner files (~$*.docx) and *.tmp files are transient, are never
    synced by OneDrive, and appear whenever a document is open, so they do NOT make a
    folder NotReady; they are counted in TotalTempFiles and called out in Summary as a
    caution to close documents before migrating.

    The OneDrive root used for projection is resolved, in order, from the signed-in
    user's business account whose ConfiguredTenantId matches $S_TenantId, then the one
    whose sync folder is named $S_OneDriveFolderName, then the configured name under the
    profile. Matching by tenant ID is what recognises users whose sync folder still
    carries an older organisation display name (the folder is named at sign-in and never
    renames itself). Every business sync folder name found is listed in OneDriveAccounts.
    ...
    columns roll those up into KfmState (AlreadyMigrated / MigratedWithIssues / Ready /
    Partial / NotReady / NotFound) and a single MigrationReady Yes/No. Folders that are
    already inside OneDrive are still checked - an invalid name there is content OneDrive
    cannot sync - but the state says the move itself has already happened. A short, character-
...
# Used when no business account can be matched from the registry (see $S_TenantId), and
# as the second-choice registry match by folder name.
$S_OneDriveFolderName = 'OneDrive - Contoso'

# Microsoft Entra tenant ID (GUID) of the OneDrive for Business tenant. Optional but
# strongly recommended: a business account whose ConfiguredTenantId matches is used as the
# OneDrive root whatever its sync folder is called, so users who signed in before the
# organisation display name changed (and still have "OneDrive - <old name>") are reported
# as already in OneDrive instead of being projected into a second folder. '' disables it.
$S_TenantId = ''

# Character cap for the OneDriveAccounts column (all business sync folder names found).
$S_MaxAccountsLength = 120
...
# Transient files OneDrive never syncs: Office owner/lock files (~$Report.docx) and .tmp
# files. Wildcards allowed. They are counted in TotalTempFiles and mentioned in Summary,
# but do not affect readiness - one open Word document must not make a device NotReady.
$S_TempFilePatterns = @('~$*', '*.tmp')
...
function Get-OneDriveBusinessAccount
{
    <#
        ...
        Folders    - every business sync folder leaf name found, for the OneDriveAccounts column.
        Root       - the sync folder to treat as the OneDrive root: the account whose
                     ConfiguredTenantId matches $S_TenantId first, else the one whose folder
                     is named $S_OneDriveFolderName, else '' (caller falls back to config).
        Source     - RegistryTenant / RegistryName / '' accordingly.
        A missing key simply means OneDrive is not set up; it is not an error.
    #>
    $F_Out = [pscustomobject]@{ AnyAccount = $false; Folders = @(); Root = ''; Source = '' }

    $F_WantTenant = ([string]$S_TenantId).Trim().Trim('{', '}')
    $F_TenantRoot = ''
    $F_NameRoot   = ''
    ...
    try
    {
        ...
        foreach ($F_SubName in ($F_Key.GetSubKeyNames() | Sort-Object))
        {
            ...
            $F_Tenant = ([string]$F_Sub.GetValue('ConfiguredTenantId')).Trim().Trim('{', '}')
            $F_Sub.Close()

            if ([string]::IsNullOrWhiteSpace($F_Folder)) { continue }

            $F_Folder = $F_Folder.TrimEnd('\')
            $F_Leaf   = [System.IO.Path]::GetFileName($F_Folder)

            $F_Out.AnyAccount = $true
            $F_Out.Folders   += $F_Leaf

            if ($F_TenantRoot -eq '' -and $F_WantTenant -ne '' -and $F_Tenant -ieq $F_WantTenant) { $F_TenantRoot = $F_Folder }
            if ($F_NameRoot   -eq '' -and $F_Leaf -ieq $S_OneDriveFolderName)                    { $F_NameRoot   = $F_Folder }
        }
    }
    catch
    {
    ...
    if ($F_TenantRoot -ne '')  { $F_Out.Root = $F_TenantRoot; $F_Out.Source = 'RegistryTenant' }
    elseif ($F_NameRoot -ne '') { $F_Out.Root = $F_NameRoot;   $F_Out.Source = 'RegistryName'   }
...
function Test-RealLink
{
    <#
        True only for a junction or symbolic link. Every OneDrive Files On-Demand
        placeholder also has the ReparsePoint attribute, so the attribute alone is not
        enough: PowerShell's LinkType reads the reparse tag and returns SymbolicLink or
        Junction for real links and nothing for cloud placeholders. If the tag cannot be
        read the cloud attribute bits (0x40000 RecallOnOpen, 0x400000 RecallOnDataAccess)
        are used as a weaker fallback.
    #>
    param([System.IO.FileSystemInfo]$F_Item)

    $F_Attr = [int]$F_Item.Attributes
    if (($F_Attr -band 0x400) -eq 0) { return $false }

    try
    {
        $F_LinkType = [string]$F_Item.LinkType
        return ($F_LinkType -eq 'SymbolicLink' -or $F_LinkType -eq 'Junction')
    }
    catch
    {
        return (($F_Attr -band 0x440000) -eq 0)
    }
}
...
function Test-TempFileName
{
    <# True when the file name matches one of $S_TempFilePatterns (case-insensitive). #>
    param([string]$F_Name)

    foreach ($F_Pattern in $S_TempFilePatterns)
    {
        if ($F_Name -like $F_Pattern) { return $true }
    }
    return $false
}
...
function Invoke-KnownFolderScan
{
    ...
    while ($F_Stack.Count -gt 0)
    {
        ...
        foreach ($F_Child in $F_Children)
        {
            ...
            if (-not $F_IsDir -and (Test-TempFileName -F_Name $F_Name)) { $F_Result.TempFiles++; continue }

            $F_Rel = $(if ($F_Dir.Rel) { "$($F_Dir.Rel)\$F_Name" } else { $F_Name })

            if (Test-RealLink -F_Item $F_Child)
            {
                $F_Result.Links++
...
try
{
    ...
    $F_Data.OneDriveAccounts = Format-ListField -F_Items $F_Account.Folders -F_MaxLength $S_MaxAccountsLength

    if ($F_Account.Root -ne '')
    {
        $F_OneDriveRoot = $F_Account.Root
        $F_Data.OneDriveRootSource = $F_Account.Source
    }
    else
    {
        $F_OneDriveRoot = [System.IO.Path]::Combine($F_ProfilePath, $S_OneDriveFolderName)
    ...
    for ($F_Index = 0; $F_Index -lt @($S_KnownFolders).Count; $F_Index++)
    {
        ...
        $F_InOneDrive = $F_Path.StartsWith($F_OneDriveRoot + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)
        $F_Data["${F_Label}_InOneDrive"] = $(if ($F_InOneDrive) { 'Yes' } else { 'No' })
        if ($F_InOneDrive) { $F_Data.FoldersInOneDrive++ }
        ...
        $F_ProjectedRoot = $(if ($F_InOneDrive) { $F_Path } else { [System.IO.Path]::Combine($F_OneDriveRoot, [System.IO.Path]::GetFileName($F_Path)) })

        $F_Scan = Invoke-KnownFolderScan -F_RootPath $F_Path -F_ProjectedRoot $F_ProjectedRoot -F_Label $F_Label `
                                         -F_UsePrefix $F_UsePrefix -F_IssueLists $F_IssueLists
        ...
        $F_Data.TotalTempFiles  += $F_Scan.TempFiles
    ...
    # KfmState: the single column to slice a rollout by.
    #   NotFound           - no known folder could be resolved at all.
    #   AlreadyMigrated    - every known folder found is inside the OneDrive root, no issues.
    #   MigratedWithIssues - every known folder found is inside the OneDrive root, but some
    #                        content cannot sync (invalid names, links). The move is done;
    #                        the content still needs fixing.
    #   NotReady           - not all folders are in OneDrive yet and something blocks the
    #                        move (or known folders are nested).
    #   Partial            - some folders are inside OneDrive, some are not; nothing blocks
    #                        the rest.
    #   Ready              - none are inside OneDrive yet and nothing blocks the move.
    $F_Found    = $F_Data.FoldersReady + $F_Data.FoldersNotReady
    $F_Migrated = ($F_Found -gt 0) -and ($F_Data.FoldersInOneDrive -eq $F_Found)

    if ($F_Found -eq 0)                          { $F_Data.KfmState = 'NotFound' }
    elseif ($F_Migrated -and $F_Ready)           { $F_Data.KfmState = 'AlreadyMigrated' }
    elseif ($F_Migrated)                         { $F_Data.KfmState = 'MigratedWithIssues' }
    elseif (-not $F_Ready)                       { $F_Data.KfmState = 'NotReady' }
    elseif ($F_Data.FoldersInOneDrive -gt 0)     { $F_Data.KfmState = 'Partial' }
    else                                         { $F_Data.KfmState = 'Ready' }

    $F_RootName   = [System.IO.Path]::GetFileName($F_OneDriveRoot)
    $F_IssueText  = "$($F_Data.TotalLongPaths) long path(s), $($F_Data.TotalBadNames + $F_Data.TotalBadFolders) invalid name(s), $($F_Data.TotalLinks) link(s)"
    if ($F_Nested) { $F_IssueText += ', nested known folders' }

    switch ($F_Data.KfmState)
    {
        'NotFound'           { $F_Summary = 'No known folders could be resolved for this user.' }
        'AlreadyMigrated'    { $F_Summary = "Already in OneDrive: $($F_Data.FoldersInOneDrive) folder(s) under $F_RootName, $($F_Data.TotalFiles) file(s) checked, no issues." }
        'MigratedWithIssues' { $F_Summary = "Already in OneDrive ($F_RootName) but content in $($F_NotReady -join ', ') cannot sync: $F_IssueText." }
        'NotReady'           { $F_Summary = "Not ready: $($F_NotReady -join ', '). $F_IssueText across $($F_Data.TotalFiles) file(s)." }
        'Partial'            { $F_Summary = "Partially migrated: $($F_Data.FoldersInOneDrive) of $F_Found folder(s) already in OneDrive; the rest are clean. $($F_Data.TotalFiles) file(s) checked." }
        default              { $F_Summary = "Ready for Known Folder Move: $($F_Data.FoldersReady) folder(s) clean, $($F_Data.FoldersNotFound) not found, $($F_Data.TotalFiles) file(s) checked." }
    }

    if ($F_Data.TotalTempFiles -gt 0) { $F_Summary += " Caution: $($F_Data.TotalTempFiles) Office temp file(s) present - close open documents before migrating." }

    $F_Status = $(if ($F_Ready) { 'Compliant' } elseif ($S_ExitNonCompliantWhenNotReady) { 'NonCompliant' } else { 'Compliant' })
